/* ============================================================
   ...
   cloneMaps (circle-packed subclones), phylogenetic lineages
   and a fine DNA texture.
...
(function () {
  ...
  // indigo tint ramp (lab accent → pale wash)
  ...
  function fishplot(seed) {
    ...
    clones.forEach(function (c, ci) {
      ...
      for (var i = 0; i <= N; i++) {
        var t = i / N;
        var g = t < c.born ? 0 : Math.min(1, (t - c.born) / 0.18); // grow-in
        // envelope: rise, plateau with wobble, gentle decline near end
  ...
  // ============================================================ CLONEMAP
  // circle-packed nested subclones (parent → child → grandchild)
  function clonemap(seed) {
    var r = mulberry32(seed || 1);
    var W = 620, H = 620;
    var circles = [];
    function place(cx, cy, R, depth, tint) {
      circles.push({ x: cx, y: cy, r: R, t: tint, depth: depth });
      if (depth >= 3 || R < 34) return;
      var kids = 2 + Math.floor(r() * 3);
      var placed = [];
      var tries = 0;
      while (placed.length < kids && tries < 120) {
        tries++;
        var kr = R * (0.28 + r() * 0.34);
        var ang = r() * 6.28, rad = r() * (R - kr - R * 0.12);
        var kx = cx + Math.cos(ang) * rad, ky = cy + Math.sin(ang) * rad;
        var ok = true;
        for (var i = 0; i < placed.length; i++) {
          var dx = kx - placed[i].x, dy = ky - placed[i].y;
          if (Math.hypot(dx, dy) < kr + placed[i].r + 6) { ok = false; break; }
        }
        if (ok) { placed.push({ x: kx, y: ky, r: kr }); place(kx, ky, kr, depth + 1, Math.min(6, tint + 1)); }
      }
    }
    place(W / 2, H / 2, 288, 0, 2);

    var out = "";
    circles.forEach(function (c, i) {
      out += '<circle cx="' + c.x.toFixed(1) + '" cy="' + c.y.toFixed(1) + '" r="' + c.r.toFixed(1) +
             '" fill="' + TINTS[c.t] + '" fill-opacity="' + (0.16 + c.depth * 0.06).toFixed(2) +
             '" stroke="' + TINTS[Math.max(0, c.t - 1)] + '" stroke-opacity="0.5" stroke-width="1.1"' +
             ' class="cm-node" style="--d:' + (i * 0.06).toFixed(2) + 's;transform-origin:' +
             c.x.toFixed(1) + 'px ' + c.y.toFixed(1) + 'px"/>';
    });
  ...
  function init() {
    document.querySelectorAll("[data-decor]").forEach(render);
  }
